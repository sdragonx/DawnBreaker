...
void GameManager::Display()
{
    ...
    // display text multilines
    this->DisplayLines();

    glutSwapBuffers();
}

void GameManager::printf(const char* param, ...)
{
    char buf[1024] = { 0 };
    va_list body;
    va_start(body, param);
...
void GameManager::DisplayLines()
{
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        glRasterPos3f(4.0f, WINDOW_HEIGHT - 20 - 12 * (20 - i) - 4.0f, 0.0f);
        for (size_t j = 0; j < lines[i].size(); ++j) {
            glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, lines[i][j]);
        }
    }
